...
% 30 Dec 2013 Commented out many lines to overplot PRNs on Grimaud's
% color-mapped skyplot.
% 31 Mar 2017 Modifying to not create new axes so this can overplot on the
% all-sky camera data. the linestyle input should be a string containing
% [linestyle color symbol].  Commented out plotting of white background
% rectangle.
% 4 Sept 2019 Again commenting out the newplot.m command to overplot on
% all-sky data.
...
hpol = [];
% Comment axis section if you don't want to clear all-sky image.
%% Prepare axis =========================================================== 
hAxis = newplot(hAxis); 
hAxis = gca; 
...
if strcmp(line_style, 'auto') 
    ...
    if ~isspace(line_style(1))
    hpol = plot(xx', yy', line_style(1:2)); 
    end
end 
 
%--- Mark the last position of the satellite ------------------------------ 
plot(xx(:,end)', yy(:,end)', line_style(2:3), 'MarkerSize', 14, ...
    'MarkerFaceColor',line_style(2)); 
...
%         text(xx(i, end), yy(i, end), ['  ', int2str(prn(i))], 'color',map2(i,:),'FontSize',14, 'FontWeight','bold'); 
        text(xx(i, end), yy(i, end), ['  ', int2str(prn(i))], 'color',tc,'FontSize',20, 'FontWeight','bold'); 
    end 
end 
end
